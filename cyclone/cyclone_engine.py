...
class Cyclone:
    ...
    async def _enrich_position_type(self, alert):
        """
        Dispatch enrichment for position-level alerts based on alert_type.
        """
        alert_type = str(alert.alert_type).lower()
        log.debug(f"🧭 Position alert type detected: {alert_type}", source="AlertEnrichment")
        ...
        try:
            if alert_type == "profit":
                ...
                log.warning(
                    f"⚠️ Unsupported alert_type '{alert_type}' for position alert {alert.id}",
                    source="AlertEnrichment"
                )
                return alert
        except Exception as e:
            log.error(f"❌ Failed to enrich position alert {alert.id}: {e}", source="AlertEnrichment")
            return alert
    ...
    async def run_cleanse_ids(self):
        ...
        # In cyclone_engine.py (Cyclone class)
    ...
    async def run_cycle(self, steps=None):
        ...
        default_order = [
            ...
            "update_evaluated_value", "alert", "system", "link_hedges", "update_hedges"
        ]
